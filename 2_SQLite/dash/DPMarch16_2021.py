...
tabmsg = {'set':'Frac_setting','acl':'All_courses list', 'cml': 'Cost model',
          'reg':'Registration', 'dem':'Demography', 'gra':'Grades', 'svy':'TOOT'}
...
#******************************************************************************
### 3.7 If table already exists, querry and filter!
#******************************************************************************
#------------------------------------------------------------------------------
def check_tables():
...
def qerrydb(dbpath, col, tabname):
    ...
    if 'course_list' in table_list:
        df = topd(engine.execute('Select DISTINCT * from '+tabname),col)
        flag = True
    else:
        df = False
    return df, flag
...
def filtab(df, dbtab, cid):
    ...
    if isinstance(df, bool)==False:
        lst1 = df[cid].tolist()
        if isinstance(ddtab, bool)==False:
...
# Create the DB
def create_tab(df, engine, tab_name, mode):# mode = 'append'/'replace'
    df.to_sql(tab_name, engine, if_exists=mode, index=False)

def tosqlite(dbpath, df,dbtab, cid, flag, tab_name,tabmsg1,mode, rplc = False):
    error = replaces= []
    if rplc == False:
    ...
    if isinstance(df, bool)==False and flag == None:
        create_tab(df, engine, tab_name, mode)
        if mode == 'append':
...
#%%%%
#tosqlite(dbpath, df, dbtab, cid, flag, tabmsg1, mode, tab_name)
#******************************************************************************
#******************************************************************************
...
def sqliteTopd(dbpath1, querycol, tabname, dupcol):
    df1, flag = qerrydb(dbpath1, querycol, tabname)
    df2 = topd(df1, querycol)
    df = remove_duplicate(df2, dupcol)
    return df, flag
#%%%
...
acl1, flagacl = qerrydb(dbpath, querycol['acl'], 'course_list')
reg1, flagreg = qerrydb(dbpath, querycol['reg'], 'registrants')
dem1, flagdem = qerrydb(dbpath, querycol['dem'], 'demography')
gra1, flaggra = qerrydb(dbpath, querycol['gra'], 'grade_report')
cml1, flagcml = qerrydb(dbpath, querycol['cml'], 'cost_model')
svy1, flagsvy = qerrydb(dbpath, querycol['svy'], 'surveys')
fset1, flagset = qerrydb(dbpath, querycol['set'], 'frac_setting')

setf = (isinstance(dfset, bool)== False)
er1, rpc1 = tosqlite(dbpath, dfall, acl1, 'acronym', allf, 'course_list', tabmsg['acl'],'append')
er2, rpc2 = tosqlite(dbpath, dfreg, reg1, 'course_id', regf, 'registrants', tabmsg['reg'],'append')
er3, rpc3 = tosqlite(dbpath, demog, dem1, 'course_id', regf, 'demography', tabmsg['dem'],'append')
er4, rpc4 = tosqlite(dbpath, dfgra, gra1, 'course_id', graf, 'grade_report', tabmsg['gra'],'append')
er5, rpc5 = tosqlite(dbpath, dfcos, cml1, 'course_id', cosf, 'cost_model', tabmsg['cml'],'append')
er6, rpc6 = tosqlite(dbpath, dfsur1, svy1, 'course_id', sur1f, 'surveys', tabmsg['svy'],'append')
er7, rpc7 = tosqlite(dbpath, dfset, fset1, 'course_id', setf, 'frac_setting', tabmsg['set'], 'replace')
errors.append(er1+er2+er3+er4+er5+er6+er7)

table_list = check_tables()
backupDb(path_4)

acl, facl = sqliteTopd(dbpath, querycol['acl'], 'course_list', dupcols['acl'])
reg, freg = sqliteTopd(dbpath, querycol['reg'], 'registrants', dupcols['reg'])
dem, fdem = sqliteTopd(dbpath, querycol['dem'], 'demography', dupcols['dem'])
cml, fcml = sqliteTopd(dbpath, querycol['cml'], 'cost_model', dupcols['cml'])
gra, fgra = sqliteTopd(dbpath, querycol['gra'], 'grade_report', dupcols['gra'])
svy, fsvy = sqliteTopd(dbpath, querycol['svy'], 'surveys', dupcols['svy'])
fset, fset = sqliteTopd(dbpath, querycol['set'], 'frac_setting', dupcols['set'])
lflags = [facl, freg, fdem, fcml, fgra, fsvy, fset]
...
def replacetab(df, tbl, tab_name):
    if isinstance(tbl, bool)==False and isinstance(df, bool)==False:
...
def replace_tables():
    errors = reptabs = []
    set_path(path_dict)
    filenames, ids, dfall, dfreg, dfgra, dfcos, dfsur, dfset,dfcklst = read_dsets(path_9)
    call_writer()
    remove_inputfiles()
    ...
    dfallrpmt = replacetab(dfall, acl1, 'course_list')
    dfregrpmt = replacetab(dfreg, reg1, 'registrants')
    demogrpmt = replacetab(demog, dem1, 'demography')
    dfgrarpmt = replacetab(dfgra, gra1, 'grade_report')
    dfsur1rpmt = replacetab(dfsur1, svy1, 'surveys')
    dfcosrpmt = replacetab(dfcos, cml1, 'cost_model')

    setf = (isinstance(dfset, bool)== False)
    er1, rpc1 = tosqlite(dbpath, dfallrpmt, acl1, 'acronym', allf, 'course_list', tabmsg['acl'],'replace', rplc = True)
    er2, rpc2 = tosqlite(dbpath, dfregrpmt, reg1, 'course_id', regf, 'registrants', tabmsg['reg'],'replace', rplc = True)
    er3, rpc3 = tosqlite(dbpath, demogrpmt, dem1, 'course_id', regf, 'demography', tabmsg['dem'],'replace', rplc = True)
    er4, rpc4 = tosqlite(dbpath, dfgrarpmt, gra1, 'course_id', graf, 'grade_report', tabmsg['gra'],'replace', rplc = True)
    er5, rpc5 = tosqlite(dbpath, dfcosrpmt, cml1, 'course_id', cosf, 'cost_model', tabmsg['cml'],'replace', rplc = True)
    er6, rpc6 = tosqlite(dbpath, dfsur1rpmt, svy1, 'course_id', sur1f, 'surveys', tabmsg['svy'],'replace', rplc = True)
    errors.append(er1+er2+er3+er4+er5+er6)
    reptabs.append(rpc1+rpc2+rpc3+rpc4+rpc5+rpc6)
    backupDb(path_4)
    if len(reptabs) == 0:
        reptabs.append('None')
...
def replace_trables(n_clicks):
    if n_clicks > 0:
        errorsr, reptabs = replace_tables()
